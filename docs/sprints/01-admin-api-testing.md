...
- 使用 `ShoplineService.getStoreByHandle()` 取得商店資訊
...
**實作模式**:
- 使用 `getStoreByHandle()` 取得商店資訊和 Access Token
...
**檔案**: `frontend/types.ts` 或 `backend/src/types.ts`

需要定義的型別：
```typescript
// Products
interface Product { ... }
interface ProductListParams { page?: number, limit?: number, ... }
interface ProductListResponse { products: Product[], pagination?: ... }
interface CreateProductInput { ... }
interface UpdateProductInput { ... }

// Orders
interface Order { ... }
interface OrderListParams { page?: number, limit?: number, status?: string, ... }
interface OrderListResponse { orders: Order[], pagination?: ... }
interface UpdateOrderInput { ... }

// Customers
interface Customer { ... }
interface CustomerListParams { page?: number, limit?: number, ... }
interface CustomerListResponse { customers: Customer[], pagination?: ... }
...
- 從資料庫讀取商店的 Access Token（使用 Sprint 0 的儲存機制）
...
- [ ] 所有 Phase 1 的 Service 方法實作完成
- [ ] 所有 Phase 1 的 API Routes 實作完成
- [ ] 錯誤處理機制完善
- [ ] 型別定義完整
- [ ] 單元測試通過率 > 80%
...
- 本 Sprint 依賴 Sprint 0 的 OAuth 授權機制和資料庫設定

---

**Sprint 狀態**: 📝 規劃中（待 Review）  
**建立日期**: 2025-01-XX  
**最後更新**: 2025-01-XX
