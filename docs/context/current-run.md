...
5. **Story 3.5** → OAuth 授權流程與會員登入系統銜接（依賴 Story 3.1, 3.2, 3.3, 3.4）
...
1. **完整 OAuth 授權流程測試**
   ...
   - 看到空白授權商店
   - 點擊新增商店授權
   - 驗證：確保使用者已登入（如果未登入，重導向到登入頁）
   - 走 OAuth 流程（跳轉到 Shopline 授權頁面）
   - 在 Shopline embedded 導回授權完成回到首頁後
   - 驗證：使用者仍然是登入狀態
   - 驗證：授權完成的商店正確關聯到當前使用者
   - 驗證：可以開始使用 API
...
2. **資料隔離測試**
   ...
   - 驗證：使用者 A 只能看到商店 A
   - 驗證：使用者 B 只能看到商店 B
   - 驗證：使用者 A 無法使用商店 B 的 API
   - 驗證：使用者 B 無法使用商店 A 的 API

3. **OAuth 回調狀態保持測試**
   - 登入後進行 OAuth 授權
   - 驗證：OAuth 回調後，使用者仍然是登入狀態
   - 驗證：授權的商店正確關聯到當前使用者
   - 驗證：商店列表自動更新
...
5. **資料隔離** ✅
   - 登出後換另一個帳戶登入，看不到前一個帳戶的資料

### 未涵蓋的測試項目

詳細記錄在：`docs/backlog/inbox/note-2025-11-06-002.md`
...
**主要問題**：
1. **註冊流程防呆機制不完整**：註冊已存在的 email 直接報 400，沒有友善的錯誤訊息
2. **前後端 state 控管不順暢**：登出後登入新帳戶會閃過幾秒鐘舊帳戶的資料才清空
