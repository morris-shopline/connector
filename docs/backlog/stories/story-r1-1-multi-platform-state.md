...
- 2025-11-10：Run `run-2025-11-10-01` 開始實作 Phase A 任務，負責 Agent：GPT-5 Codex。
- 2025-11-10：完成 Connection 狀態重構（Zustand store、Router helper、localStorage cache），等待自動化與手動測試覆核。
- 2025-11-10：**修復登出流程** - User Test 發現登出後登入新帳號仍能看到舊資料：
  - ✅ 在登出時清除所有 SWR 快取（`mutate(() => true, undefined, { revalidate: false })`）
  - ✅ 在登入時也清除所有舊的快取和狀態（確保不會顯示舊帳號的資料）
  - ✅ 清除 URL query 參數
  - 修改檔案：`frontend/components/Header.tsx`、`frontend/pages/login.tsx`
...
- 建立 `frontend/hooks/useConnectionRouting.ts`（供 R3.1 繼承）：
  ...
  - `applyConnection(params)` → 更新 Zustand，然後推送 URL（單向推送）
...
- [x] Kickoff Review 完成並於 run 文件記錄結論（2025-11-10）
- [x] Zustand Store 新欄位與 action 實作完成，型別檢查通過（2025-11-10）
- [x] State 分層策略落地（Zustand 是唯一 Source of Truth，URL 只用來初始化），E2E 測試覆蓋登入/登出/授權流程（2025-11-10）
- ⚠️ **修正（2025-11-10）**：「Router Query 單一來源」改為「State 分層策略」，避免 Dual Source of Truth
- [x] localStorage 快取依據 helper 實作，提供 reset 與版本檢查（2025-11-10）
- [x] Issue 2025-11-06-001 狀態更新為 `resolved`（2025-11-10）
- [x] Story 文件測試清單全部勾選，User Test 通過（2025-11-10）
...
- **僅帶 `connectionItemId` 的 URL**
  ...
  - 確認頁面自動補齊 `platform` / `connectionId` 並更新 Zustand
...
- **登出流程**
  ...
  - ✅ **修復（2025-11-10）**：登出時清除所有 SWR 快取，避免登入新帳號時顯示舊資料
  - ✅ **修復（2025-11-10）**：登入時也清除所有舊的快取和狀態
- **多分頁行為（可選）**
