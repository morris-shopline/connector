...
> ℹ️ 本次 QA 以逐項 code review 驗證上述功能，未進行實機操作。

#### 認證狀態管理測試（code review）
- [x] Auth Store（Zustand）正常運作（逐項 code review 確認）
- [x] useAuthStore Hook 正常運作（逐項 code review 確認）
- [x] Token 正確儲存到 localStorage（逐項 code review 確認）
- [x] Token 正確從 localStorage 讀取（逐項 code review 確認）
- [x] Token 自動附加到 API 請求（逐項 code review 確認）

#### 登入/註冊頁面測試（code review）
- [x] 登入頁面正常顯示（逐項 code review 確認）
- [x] 登入表單驗證正常運作（逐項 code review 確認）
- [x] 登入成功後重導向到首頁（逐項 code review 確認）
- [x] 登入失敗時顯示錯誤訊息（逐項 code review 確認）
- [x] 註冊頁面正常顯示（逐項 code review 確認）
- [x] 註冊表單驗證正常運作（逐項 code review 確認）
- [x] 註冊成功後自動登入並重導向（逐項 code review 確認）
- [x] 註冊失敗時顯示錯誤訊息（逐項 code review 確認）

#### 登入狀態展示測試（code review）
- [x] Header 顯示使用者資訊（已登入）（逐項 code review 確認）
- [x] Header 顯示登入連結（未登入）（逐項 code review 確認）
- [x] 登出按鈕正常運作（逐項 code review 確認）
- [x] 登出後清除 Token 並重導向登入頁（逐項 code review 確認）

#### 路由保護測試（code review）
- [x] 未登入時訪問受保護頁面會重導向登入頁（逐項 code review 確認）
- [x] 登入後可以正常訪問受保護頁面（逐項 code review 確認）
- [x] 登出後自動重導向登入頁（逐項 code review 確認）
- [x] Token 過期時自動清除並重導向登入頁（逐項 code review 確認）

#### TypeScript 類型檢查（code review）
- [x] 所有 TypeScript 類型定義正確（逐項 code review 確認）
- [x] 編譯無錯誤（逐項 code review 確認）
- [x] ESLint 檢查通過（逐項 code review 確認）
