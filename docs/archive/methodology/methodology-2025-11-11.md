...
- `planning` - Story 開始規劃，Agent 正在整理資訊（開始建立 Story 文件）
- `planned` - Agent 整理完一輪，Story 文件已建立（包含技術需求、驗收標準、User Test 步驟等，但尚未經過用戶確認）
- `ready-for-dev` - 用戶確認沒問題，可以開始開發（用戶確認 Story 文件無問題，可以安排進 Run）
- `in-development` - Story 正在開發中
...
- 建立 Epic 文件
- 列出初步的 Story 清單（狀態：`not-started`）

**Story 狀態標記**：
- Epic 規劃階段時，Story 文件尚未建立，狀態應標記為 `not-started`
- 當開始建立 Story 文件時，狀態轉換為 `planning`
- Agent 整理完一輪後，狀態轉換為 `planned`
- 用戶確認沒問題後，狀態轉換為 `ready-for-dev`
...
6. **狀態調整與檢查**
   - **狀態轉換流程**：
     - `not-started` → `planning`：開始建立 Story 文件時，將狀態轉換為 `planning`
     - `planning` → `planned`：Agent 整理完一輪（包含技術需求、驗收標準、User Test 步驟等），將狀態轉換為 `planned`
     - `planned` → `ready-for-dev`：用戶確認 Story 文件沒問題後，將狀態轉換為 `ready-for-dev`
   - 若仍有資料缺漏或測項未定義，維持當前狀態並補齊缺口
   - 只有當 Story 狀態為 `ready-for-dev` 時，才能安排進 Run
